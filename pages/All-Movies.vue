...
<div>
    <v-container>
        <ApolloQuery 
            :query="require('../gql/queries/movies.gql')"> 
                <template v-slot="{ result: { loading, error, data } }">
                    ...
                    <div v-else-if="data" class="result apollo">
                      ...
                      <div><br>
                      <v-row>
                          <v-col cols="6"><h1>All Movies</h1></v-col>
                      </v-row> 

                         <br>  
                        <v-row>
                            <v-col cols="3" justify-space-around mb-6  v-for="all in data.Movies " :key="all.Title">
                                <v-card :loading="loading" style="border-radius: 10px" 
                                    class="mx-auto"
                                    max-width="200" 
                                    color="transparent" 
                                    @click="goTo(all.Title)"
                                >
                                    <v-img
                                        :src="all.Path_Poster"
                                        height="400px"
                    ...
                    {{all.Rating}} <span><v-icon small color="orange">mdi-star</v-icon></span> 
                  </v-avatar></v-img>
                                    <h3>
                  {{all.Title}} </h3><hr>
